...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link or 'tg://openmessage' in msg_link:
        ...
        if chat in saved_channel_ids:
            await app.edit_message_text(message.chat.id, edit_id, "Sorry! This channel is protected 🔐 By **JarvisBotz**")
            return
...
def get_user_rename_preference(user_id):
    # Retrieve the user's custom renaming tag if set, or default to 'JarvisBotz'
    return user_rename_preferences.get(str(user_id), 'JarvisBotz')
...
async def settings_command(event):
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/+8mFE3KUGXUo0NmY0")]
    ]
